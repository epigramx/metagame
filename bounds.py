...
def setupBasicProblem(matrix):
    prob = p.LpProblem("rock_paper_scissors", p.LpMaximize)
    the_vars = np.append(matrix.index.values, (["w"]))
    lp_vars = p.LpVariable.dicts("vrs", the_vars)
#First add the objective function.
    prob += p.lpSum([lp_vars['w']])
#Now add the non-negativity constraints.
    for row_strat in matrix.index.values:
        prob += p.lpSum([1.0 * lp_vars[row_strat]]) >= 0
#Now add the sum=1 constraint.
    prob += p.lpSum([1.0 * lp_vars[x] for x in matrix.index.values]) == 1
...
#now prepare the value and mixed strategy
    game_val = p.value(lp_vars['w'])
    strat_probs = {}
    for row_strat in matrix.index.values:
        strat_probs[row_strat] = p.value(lp_vars[row_strat])
#and output it
...
#add the additional constraint
    prob += p.lpSum(lp_vars[rowname]) == constraint
    prob.writeLP("rockpaperscissors.lp")
    prob.solve()
#now prepare the value and mixed strategy
    game_val = p.value(lp_vars['w'])
